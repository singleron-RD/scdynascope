...
class ConversionMerge:
    ...
    def merge_csv(self):
        ...
        self.df_conv = df.groupby(["chrom", "pos"]).agg({"convs": "sum", "covers": "sum"}).reset_index()
...
def get_opts_conversion():
    ...
    parser.add_argument(
        "--snp_threshold", type=float, help="snp threshold filter, greater than snp_threshold will be recognized as snp"
    )
    parser.add_argument("--snp_min_depth", type=int, help="Minimum depth to call a variant")
